...
Ext.define('TipToast.Toast', {requires:['Ext.dom.Helper'], alternateClassName:['TipToast'], singleton:true, msgCt:null, divId:'tip-toast-msg-div', direction:'top', options:null, config:{delay:2000, size:'default', position:'default', remove:true, closeable:true, cls:''}, constructor:function(config) {
  ...
  if (this.msgCt) {
    ...
    this.msgCt = Ext.DomHelper.append(document.body, {id:this.divId, cls:direction + ' ' + size + ' ' + position}, true);
  }
  ...
  this.toast(title, message, options);
}, privates:{createBox:function(title, msg, options) {
  var boxCls = options.cls;
  var closeBtn = Ext.String.format('\x3cbtn tip-toast-close-btn\x3d"1" aria-hidden\x3d"true" ' + 'class\x3d"close tip-toast-close-btn tip-toast-close-btn-{0}" ' + 'style\x3d"position: relative; float: right; right: -10px;top: -10px; z-index: 20000;"\x3e×\x3c/btn\x3e', options.closeable === true ? 'show' : 'hidden');
  return Ext.String.format('\x3cdiv class\x3d"tip-toast msg {3} {0}border-box"\x3e' + closeBtn + '\x3ch3\x3e{1}\x3c/h3\x3e' + '\x3cp\x3e{2}\x3c/p\x3e' + '\x3c/div\x3e', Ext.baseCSSPrefix, title, msg, boxCls ? boxCls : '');
  ...
  this.options = {delay:this.getDelay(), size:this.getSize(), position:this.getPosition(), closeable:this.getCloseable(), remove:this.getRemove(), cls:this.getCls()};
}}});
